...
def test_clone_scan_retention() -> None:
    ...
    print(f"[OK] Cloned to {WorkspaceManager.temp_label(cloned)}")
    assert cloned.name.startswith("sentinel_")
    ...
    assert cloned.is_dir(), "workspace should remain for deploy TTL"
    WorkspaceManager.release_workspace(scan_id)
